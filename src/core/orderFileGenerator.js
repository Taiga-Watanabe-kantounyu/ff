...
async function generateOrderFile(orderFilePath) {
  try {
    ...
    // H3セルに本日日付を設定（年月日の形式）
    worksheet.getCell('H3').value = `${todayParts.year}年${todayParts.month}月${todayParts.day}日`;
    
    // H4セルに集荷日を設定（年月日の形式）
    worksheet.getCell('H4').value = `${pickupDateParts.year}年${pickupDateParts.month}月${pickupDateParts.day}日`;
    ...
    for (const row of rawData) {
      // 各項目を取得
      const temperatureZone = row['温度帯'] || '';
      const productCode = row['品番'] || '';
      ...
      // 値だけを設定
      currentRow.getCell(1).value = formatDateShort(deliveryDate); // A列に配送日（集荷日+リードタイム）をMM/DD形式で設定
      currentRow.getCell(2).value = temperatureZone; // 温度帯
      currentRow.getCell(3).value = productCode; // 品番
      currentRow.getCell(4).value = productName; // 品名
      currentRow.getCell(5).value = rank; // ランク
      currentRow.getCell(6).value = weight; // 重量（kg）
      currentRow.getCell(7).value = quantity; // 数量
      currentRow.getCell(8).value = deliveryName; // お届け先名
      currentRow.getCell(9).value = deliveryInfo.phone; // 届け先電話番号
      currentRow.getCell(10).value = deliveryInfo.address; // 届け先住所
    ...
    // 発注ファイル名を生成
    // 元のファイル名からメッセージIDとタイムスタンプを含む部分を抽出
    const originalFileName = path.basename(orderFilePath, path.extname(orderFilePath));
    
    // 元のファイル名に既にメッセージIDとタイムスタンプが含まれている場合は、それを保持
    // そうでない場合は、単純に「元のファイル名_発注.xlsx」という形式にする
    const outputFileName = `${originalFileName}_発注.xlsx`;
...
async function processOrderFile(orderFilePath, printAfterGeneration = true) {
  try {
    ...
    const outputFilePath = await generateOrderFile(orderFilePath);
    
    // 発注ファイル生成後に印刷処理を実行
    if (printAfterGeneration) {
      try {
        console.log(`発注ファイルの印刷を開始します: ${outputFilePath}`);
        await printFile(outputFilePath);
        console.log('印刷処理が完了しました');
      } catch (printError) {
        console.error(`印刷処理中にエラーが発生しました: ${printError.message}`);
        // 印刷エラーがあっても処理は続行し、生成されたファイルパスを返す
      }
    }
...
if (require.main === module) {
  ...
  processOrderFile(orderFilePath)
    .then(outputFilePath => {
      console.log(`処理が完了しました。発注ファイル: ${outputFilePath}`);
    })
    .catch(err => {
